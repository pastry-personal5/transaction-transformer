...
def append_transactions_of_current_date(list_of_simple_transactions: list, transactions_of_current_date: list):
    for t in transactions_of_current_date:
        if t.transaction_type == SimpleTransaction.SimpleTransactionTypeEnum.TYPE_STOCK_SPLIT_MERGE_DELETION:
            list_of_simple_transactions.append(t)
    for t in transactions_of_current_date:
        if t.transaction_type == SimpleTransaction.SimpleTransactionTypeEnum.TYPE_STOCK_SPLIT_MERGE_INSERTION:
            list_of_simple_transactions.append(t)
    for t in transactions_of_current_date:
...
def get_list_of_simple_transactions_from_stream(input_stream, account):
    ...
    for input_row in reader:
        ...
        if input_row[4] == STRING_FOR_TYPE_SELL:
            transaction.transaction_type = SimpleTransaction.SimpleTransactionTypeEnum.TYPE_SELL
        elif input_row[4] == STRING_FOR_TYPE_BUY:
            transaction.transaction_type = SimpleTransaction.SimpleTransactionTypeEnum.TYPE_BUY
