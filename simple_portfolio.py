...
class SimplePortfolio():
    ...
    def __str__(self):
        ...
        for key in self.p.keys():
            amount = self.p[key]['amount']
            open_price = self.p[key]['open_price']
            open_date = self.p[key]['open_date']
            to_return += f'symbol({key}) amount({amount}) open_price({open_price}) open_date({open_date})'
            to_return += '\n'
        return to_return
    ...
    def record(self, transaction: SimpleTransaction):
        if transaction.type == SimpleTransaction.TYPE_BUY:
            self.record_buy(transaction)
        elif transaction.type == SimpleTransaction.TYPE_SELL:
            self.record_sell(transaction)
        elif transaction.type == SimpleTransaction.TYPE_STOCK_SPLIT_MERGE_INSERTION:
            self.record_stock_split_merge_insertion(transaction)
        elif transaction.type == SimpleTransaction.TYPE_STOCK_SPLIT_MERGE_DELETION:
