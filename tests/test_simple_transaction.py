...
class TestSimpleTransaction(unittest.TestCase):
    ...
    def test_transaction_type_string(self):
        transaction_sell = SimpleTransaction(
            transaction_type=SimpleTransaction.SimpleTransactionTypeEnum.TYPE_SELL
        )
        self.assertEqual(transaction_sell.get_transaction_type_string(), "SELL")

        transaction_buy = SimpleTransaction(
            transaction_type=SimpleTransaction.SimpleTransactionTypeEnum.TYPE_BUY
        )
        self.assertEqual(transaction_buy.get_transaction_type_string(), "BUY")

        transaction_split_insertion = SimpleTransaction(
            transaction_type=SimpleTransaction.SimpleTransactionTypeEnum.TYPE_STOCK_SPLIT_MERGE_INSERTION
        )
        self.assertEqual(
            transaction_split_insertion.get_transaction_type_string(),
            "STOCK_SPLIT_MERGE_INSERTION",
        )

        transaction_split_deletion = SimpleTransaction(
            transaction_type=SimpleTransaction.SimpleTransactionTypeEnum.TYPE_STOCK_SPLIT_MERGE_DELETION
        )
        self.assertEqual(
            transaction_split_deletion.get_transaction_type_string(),
    ...
    def test_str(self):
        ...
        self.assertEqual(
            str(transaction),
            "symbol(AAPL) transaction_type(BUY) open_price(150.0) amount(100.0) commission(10.0) open_date(2023-06-12)",
        )
