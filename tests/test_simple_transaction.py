...
class TestSimpleTransaction(unittest.TestCase):
    ...
    def test_initialization(self):
        transaction = SimpleTransaction(
            symbol='AAPL',
            transaction_type=SimpleTransaction.TYPE_BUY,
            amount=100.0,
            open_price=150.0,
        ...
        self.assertEqual(transaction.transaction_type, SimpleTransaction.TYPE_BUY)
    ...
    def test_transaction_type_string(self):
        transaction_sell = SimpleTransaction(transaction_type=SimpleTransaction.TYPE_SELL)
        self.assertEqual(transaction_sell.get_transaction_type_string(), 'SELL')

        transaction_buy = SimpleTransaction(transaction_type=SimpleTransaction.TYPE_BUY)
        self.assertEqual(transaction_buy.get_transaction_type_string(), 'BUY')

        transaction_split_insertion = SimpleTransaction(transaction_type=SimpleTransaction.TYPE_STOCK_SPLIT_MERGE_INSERTION)
        self.assertEqual(transaction_split_insertion.get_transaction_type_string(), 'TYPE_STOCK_SPLIT_MERGE_INSERTION')

        transaction_split_deletion = SimpleTransaction(transaction_type=SimpleTransaction.TYPE_STOCK_SPLIT_MERGE_DELETION)
        self.assertEqual(transaction_split_deletion.get_transaction_type_string(), 'TYPE_STOCK_SPLIT_MERGE_DELETION')
