...
class TestSimplePortfolio(unittest.TestCase):
    ...
    def test_record_buy(self):
        transaction = SimpleTransaction('AAPL', SimpleTransaction.TYPE_BUY, 10, 150.0, date(2024, 6, 11), 0.0)
    ...
    def test_record_sell(self):
        transaction = SimpleTransaction('AAPL', SimpleTransaction.TYPE_BUY, 10, 150.0, date(2024, 6, 11), 0.0)
        self.portfolio.record_buy(transaction)
        transaction = SimpleTransaction('AAPL', SimpleTransaction.TYPE_SELL, 5, 155.0, date(2024, 6, 12), 0.0)
    ...
    def test_record_stock_split_merge_insertion(self):
        transaction = SimpleTransaction('AAPL', SimpleTransaction.TYPE_STOCK_SPLIT_MERGE_INSERTION, 2, 140.0, date(2024, 6, 13), 0.0)
